...
export default function TransferToken({ navigation }) {
    ...
    const handleTransfer = async () => {
        ...
        setLoading(true);
        try {
            const { data } = await axios.post(endpoints.wallet.transferToken(), {
            ...
            const errorMessage = error?.response?.data?.message || error?.response?.data || "Không thể kết nối đến server";
